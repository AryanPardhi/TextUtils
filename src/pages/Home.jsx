            ...
            <div className="mt-3 mx-5">
                <div className='d-flex align-items-center justify-content-between'>
                    ...
                    <div className="topbtn">
                        ...
                        <button type="button" disabled={text.length===0}  onClick={copy} className="btn btn-outline-info"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-clipboard-plus-fill" viewBox="0 0 16 16">
                ...
                <button type="button" disabled={text.length===0} className="btn btn-primary mt-4" onClick={upperCase}>To Uppercase</button>
                <button type="button" disabled={text.length===0} className="btn btn-secondary mt-4 mx-2" onClick={lowerCase}>To Lowercase</button>
            </div>
            <div className="textDesc mt-4 mx-5">
                <h3 className={`text-${props.mode==='dark'?'light':'dark'}`}>Text Description</h3>
                <p className={`text-${props.mode==='dark'?'danger':'success'} fs-5`}>Words {text.split(/\s+/).filter((elem)=>{
                    return elem.length !==0
                }).length} & Characters including Spaces {text.length}</p>
            </div>
